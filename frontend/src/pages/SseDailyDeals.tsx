/**
 * SseDailyDeals Page Component
 * Displays Shanghai Stock Exchange daily transaction details
 */

import React, { useState, useEffect } from 'react';
import apiClient from '@/services/api';

interface SSEDailyDealsRaw {
  [key: string]: string | number;
}

const SseDailyDeals: React.FC = () => {
  const [data, setData] = useState<SSEDailyDealsRaw[]>([]);
  ...
  const fetchSSEDailyDeals = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await apiClient.get('/market/sse-daily-deals');
      ...
      console.error('Failed to fetch SSE daily deals:', err);
