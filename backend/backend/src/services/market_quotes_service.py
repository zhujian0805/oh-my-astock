...
class MarketQuotesService:
    ...
    def _extract_bid_ask_levels(self, df) -> Dict[str, Any]:
        ...
        # Extract bid/ask levels based on akshare format
        # akshare uses: buy_1, buy_1_vol, sell_1, sell_1_vol, etc.
        for level in range(1, 6):
            ...
            if bid_price_key in data:
                levels_data[f'bid_price_{level}'] = self._parse_numeric(data[bid_price_key])
            if bid_volume_key in data:
                levels_data[f'bid_volume_{level}'] = self._parse_numeric(data[bid_volume_key], int)
            ...
            if ask_price_key in data:
                levels_data[f'ask_price_{level}'] = self._parse_numeric(data[ask_price_key])
            if ask_volume_key in data:
                levels_data[f'ask_volume_{level}'] = self._parse_numeric(data[ask_volume_key], int)
