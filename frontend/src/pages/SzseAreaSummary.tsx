/**
 * SzseAreaSummary Page Component
 * Displays regional trading rankings for Shenzhen Stock Exchange
 */

import React, { useState, useEffect } from 'react';
import apiClient from '@/services/api';

interface SZSEAreaSummaryRaw {
  [key: string]: string | number;
}

const SzseAreaSummary: React.FC = () => {
  const [data, setData] = useState<SZSEAreaSummaryRaw[]>([]);
  const [columns, setColumns] = useState<string[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    fetchSZSEAreaSummary();
  }, []);

  const fetchSZSEAreaSummary = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await apiClient.get('/market/szse-area-summary');
      ...
      console.error('Failed to fetch SZSE area summary:', err);
...
export default SzseAreaSummary;